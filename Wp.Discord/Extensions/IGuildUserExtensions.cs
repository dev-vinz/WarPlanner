...
namespace Wp.Discord.Extensions
{
    public static class IGuildUserExtensions
    {
        ...
        /// <summary>
        /// Indicates wether a guild user is a player from the competition or not
        /// </summary>
        /// <param name="user">An user to be check</param>
        /// <param name="competitionName">The competition name</param>
        /// <returns>True if the user is a player from the competition; False otherwise</returns>
        public static bool IsACompetitionPlayer(this IGuildUser user, string competitionName)
        {
            ulong[] roleIds = user.Guild.Roles
                .Where(r => r.Name.Contains(competitionName, StringComparison.InvariantCultureIgnoreCase))
                .Select(r => r.Id)
                .ToArray();

            return user.RoleIds.Any(id => roleIds.Any(i => id == i)) && user.IsAPlayer();
        }
    }
}
