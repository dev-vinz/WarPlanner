...
namespace Wp.Bot.Modules.ApplicationCommands.Manager
{
    ...
    public class War : InteractionModuleBase<SocketInteractionContext>
    {
        ...
        public async Task Add(
            [Summary("opponent", "The opponent clan's tag")] string opponentTag,
            [
                Summary("date", "When the war should be"),
                Autocomplete(typeof(WarDateAutocompleteHandler))
            ]
                string date,
            [
                Summary("preparation", "How long the preparation time should be"),
                Autocomplete(typeof(WarPreparationAutocompleteHandler))
            ]
                int warPreparation,
            [
                Summary("war", "How long the war time should be"),
                Autocomplete(typeof(WarDurationAutocompleteHandler))
            ]
                int warDuration
        )
        {
            ...
            // Filters for guild
            Calendar? dbCalendar = calendars.AsParallel().FirstOrDefault(c => c.Guild == dbGuild);
            ...
            DateTimeOffset userDate = new NodaConverter().ConvertDateTo(
                DateTimeOffset.Parse(date),
                dbGuild.TimeZone
            );
            DateTimeOffset warDate = new(userDate.Date, userDate.Offset);
            date = warDate.ToString();
            ...
            if (cOpponent is null)
            {
                await ModifyOriginalResponseAsync(
                    msg => msg.Content = generalResponses.ClashOfClansError
                );
            ...
            {
                await ModifyOriginalResponseAsync(
                    msg => msg.Content = adminResponses.CalendarIdNotSet
                );
            ...
            {
                await ModifyOriginalResponseAsync(
                    msg => msg.Content = commandText.WarAddNoCompetition
                );
            ...
            {
                await ModifyOriginalResponseAsync(
                    msg => msg.Content = commandText.WarAddNoPlayers(dbGuild.MinimalTownHallLevel)
                );
            ...
            SelectMenuBuilder menuBuilder = new SelectMenuBuilder().WithCustomId(
                IdProvider.WAR_ADD_SELECT_HOUR
            );
                ...
                .ForEach(
                    n =>
                        menuBuilder.AddOption(
                            warDate.AddHours(n).ToString("t", cultureInfo),
                            n.ToString(),
            ...
            {
                msg.Content = commandText.WarAddChooseHour(
                    cOpponent.Name,
        ...
        [SlashCommand(
            "delete",
            "Delete a registered war from the calendar",
        ...
        public async Task Delete()
        {
            ...
            {
                await ModifyOriginalResponseAsync(
                    msg => msg.Content = commandText.WarDeleteNoEvents
                );
            ...
            // Build select menu
            SelectMenuBuilder menuBuilder = new SelectMenuBuilder().WithCustomId(
                IdProvider.WAR_DELETE_SELECT_EVENT
            );
            ...
            events
                ...
                .ForEach(e =>
                {
                    ...
                    menuBuilder.AddOption(
                        $"{e.CompetitionName} : {e.OpponentClan.Name}",
                        e.Id,
                        commandText.WarDeleteMatchFromTo(
                            start.ToString("dd/MM", cultureInfo),
                            start.ToString("HH:mm", cultureInfo),
                            end.ToString("HH:mm", cultureInfo)
                        ),
                        CustomEmojis.WarSwords
                    );
                });
        ...
        public async Task Edit()
        {
            ...
            // Build select menu
            SelectMenuBuilder menuBuilder = new SelectMenuBuilder().WithCustomId(
                IdProvider.WAR_EDIT_SELECT_CHOOSE_UPDATE
            );
            ...
            events
                ...
                .ForEach(e =>
                {
                    ...
                    menuBuilder.AddOption(
                        $"{e.CompetitionName} : {e.OpponentClan.Name}",
                        e.Id,
                        commandText.WarEditMatchFromTo(
                            start.ToString("dd/MM", cultureInfo),
                            start.ToString("HH:mm", cultureInfo),
                            end.ToString("HH:mm", cultureInfo)
                        ),
