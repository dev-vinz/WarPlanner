...
namespace Wp.Bot.Modules.TimeEvents.War
{
    public static class End
    {
        ...
        public static void Execute(IGuild guild)
        {
            ...
            // Filters for guild
            Clan[] dbClans = clans.AsParallel().Where(c => c.Guild == dbGuild).ToArray();
        ...
        private static async Task DisplayResultAsync(
            IGuild guild,
            Competition? competition,
            ClashOfClans.Models.ClanWar cWar
        )
        {
            if (competition == null)
                return;
            ...
            List<FileAttachment> attachments =
                new() { await GetMainResult(cWar, competition.Guild) };

            // Gets additional images
            int nbPages = (int)Math.Ceiling((cWar.TeamSize - 5) / 10.0);
            for (int k = 0; k < nbPages; k++)
                attachments.Add(await GetAdditionnalResult(cWar, k));
        ...
        private static async Task<(bool isEsport, CalendarEvent? match)> GetEsportInformationsAsync(
            ulong guildId,
            ClashOfClans.Models.ClanWar cWar
        )
        {
            ...
            if (preparationTime.TotalHours == DefaultParameters.DEFAULT_HOUR_WAR_PREPARATION_TIME)
                return (false, null);

            Common.Models.Calendar? dbCalendar = Context.Calendars.FirstOrDefault(
                c => c.Guild.Id == guildId
            );

            Competition[] dbCompetitions = Context
                .Competitions.Where(c => c.Guild.Id == guildId)
                .ToArray();

            if (dbCalendar == null)
                return (false, null);

            DateTimeOffset startWar =
                new(cWar.PreparationStartTime.ToUniversalTime(), TimeZoneInfo.Utc.BaseUtcOffset);
            DateTimeOffset endWar =
                new(cWar.EndTime.ToUniversalTime(), TimeZoneInfo.Utc.BaseUtcOffset);

            IEnumerable<CalendarEvent> allEvents = await GoogleCalendarApi.Events.ListAsync(
                dbCalendar.Id,
                false
            );
            ...
            IEnumerable<CalendarEvent> matchStart = allEvents.Where(
                e =>
                    startWar.AddMinutes(-MINUTES_DELAY_ACCEPTED) <= e.Start.UtcDateTime
                    && e.Start.UtcDateTime <= startWar.AddMinutes(MINUTES_DELAY_ACCEPTED)
            );

            // e.EndWar € [endWar - 15; endWar + 15]
            // endWar - 15 <= e.EndWar <= endWar + 15
            IEnumerable<CalendarEvent> matchEnd = matchStart.Where(
                e =>
                    endWar.AddMinutes(-MINUTES_DELAY_ACCEPTED) <= e.End.UtcDateTime
                    && e.End.UtcDateTime <= endWar.AddMinutes(MINUTES_DELAY_ACCEPTED)
            );
            ...
            IEnumerable<CalendarEvent> matchTournament = matchEnd
                ...
                .Where(
                    e =>
                        dbCompetitions.Any(
                            c =>
                                c.Name == e.CompetitionName
                                && (c.MainTag == cWar.Clan.Tag || c.SecondTag == cWar.Clan.Tag)
                        )
                );

            if (!matchTournament.Any())
                return (false, null);

            return (true, matchTournament.First());
        }

        private static async Task<FileAttachment> GetAdditionnalResult(
            ClashOfClans.Models.ClanWar cWar,
            int index
        )
        {
            ...
            using Stream bytes = await client.GetStreamAsync(
                Utilities.SECOND_BACKGROUND_WAR_RESULT
            );
            Image image = Image.Load(bytes);

            // Initializes some settings
            Pen blackPen = new SolidPen(Color.Black, 1);
            ...
            image.Mutate(ctx =>
            {
                ...
                (
                    ClashOfClans.Models.ClanWarMember First,
                    ClashOfClans.Models.ClanWarMember Second
                )[] players = cWar.Clan.Members?.OrderBy(m => m.MapPosition)
                    .Skip(index * 10 + 5)
                    .Take(10)
                    .Zip(
                        cWar.Opponent.Members?.OrderBy(m => m.MapPosition)
                            .Skip(index * 10 + 5)
                            .Take(10) ?? Enumerable.Empty<ClashOfClans.Models.ClanWarMember>()
                    )
                    .ToArray()!;
                ...
                players
                    ...
                    .ForAll(tuple =>
                    {
                        ...
                        int clanStars =
                            clanPlayer.Attacks?.Aggregate(0, (acc, a) => acc += a.Stars) ?? 0;
                        for (int k = 0; k < clanStars; k++)
                            ctx.DrawImage(starImage, new Point(265 + k * 25, 80 + factor * 60), 1);

                        int opponentStars =
                            opponentPlayer.Attacks?.Aggregate(0, (acc, a) => acc += a.Stars) ?? 0;
                        for (int k = 0; k < opponentStars; k++)
                            ctx.DrawImage(starImage, new Point(995 - k * 25, 80 + factor * 60), 1);

                        // Draw percent
                        ctx.DrawText(
                            $"{clanPlayer.Attacks?.Aggregate(0, (acc, a) => acc += a.DestructionPercentage):#0.00}%",
                            font,
                            Color.White,
                            new(460, 80 + factor * 60)
                        );
                        ctx.DrawText(
                            new RichTextOptions(font)
                            {
                                HorizontalAlignment = HorizontalAlignment.Right,
                                Origin = new(825, 80 + factor * 60)
                            },
                            $"{opponentPlayer.Attacks?.Aggregate(0, (acc, a) => acc += a.DestructionPercentage):#0.00}%",
                            Color.White
                        );
                    });
            });
        ...
        private static async Task<FileAttachment> GetMainResult(
            ClashOfClans.Models.ClanWar cWar,
            Guild guild
        )
        {
            ...
            image.Mutate(ctx =>
            {
                ...
                ctx.DrawText(
                    $"{cWar.Clan.DestructionPercentage:#0.00}%",
                    fontInfos,
                    color,
                    new(305, 165)
                );
                ctx.DrawText(
                    $"{clanDuration:%m}{guild.GeneralResponses.ShortcutMinute} {clanDuration:ss}{guild.GeneralResponses.ShortcutSecond}",
                    fontInfos,
                    color,
                    new(305, 195)
                );
                ...
                ctx.DrawText(
                    new RichTextOptions(fontInfos)
                    {
                        HorizontalAlignment = HorizontalAlignment.Right,
                        Origin = new(975, 165)
                    },
                    $"{cWar.Opponent.DestructionPercentage:#0.00}%",
                    color
                );
                ctx.DrawText(
                    new RichTextOptions(fontInfos)
                    ...
                    $"{opponentDuration:%m}{guild.GeneralResponses.ShortcutMinute} {opponentDuration:ss}{guild.GeneralResponses.ShortcutSecond}",
                    color
                );
                ...
                (
                    ...
                    .Take(5)
                    .Zip(
                        cWar.Opponent.Members?.OrderBy(m => m.MapPosition).Take(5)
                            ?? Enumerable.Empty<ClashOfClans.Models.ClanWarMember>()
                    )
                    .ToArray()!;
                ...
                players
                    ...
                    .ForAll(tuple =>
                    {
                        ...
                        ctx.DrawText(
                            ...
                            font,
                            Color.White,
                            new(50, 308 + factor * 60)
                        );
                        ctx.DrawText(
                            new RichTextOptions(font)
                            {
                                HorizontalAlignment = HorizontalAlignment.Right,
                                Origin = new(1235, 308 + factor * 60)
                            },
                            opponentPlayer.Name,
                            Color.White
                        );
                        ...
                        int clanStars =
                            clanPlayer.Attacks?.Aggregate(0, (acc, a) => acc += a.Stars) ?? 0;
                        for (int k = 0; k < clanStars; k++)
                            ctx.DrawImage(starImage, new Point(265 + k * 25, 308 + factor * 60), 1);

                        int opponentStars =
                            opponentPlayer.Attacks?.Aggregate(0, (acc, a) => acc += a.Stars) ?? 0;
                        for (int k = 0; k < opponentStars; k++)
                            ctx.DrawImage(starImage, new Point(995 - k * 25, 308 + factor * 60), 1);
                        ...
                        ctx.DrawText(
                            ...
                            font,
                            Color.White,
                            new(460, 308 + factor * 60)
                        );
                        ctx.DrawText(
                            new RichTextOptions(font)
                            {
                                HorizontalAlignment = HorizontalAlignment.Right,
                                Origin = new(825, 308 + factor * 60)
                            },
                            $"{opponentPlayer.Attacks?.Aggregate(0, (acc, a) => acc += a.DestructionPercentage):#0.00}%",
                            Color.White
                        );
        ...
        private static void InsertPlayerStatistics(
            Guild guild,
            ClashOfClans.Models.ClanWar cWar,
            WarType warType
        )
        {
            ...
            DateTimeOffset startWar =
                new(cWar.PreparationStartTime, TimeZoneInfo.Utc.BaseUtcOffset);

            IReadOnlyCollection<ClashOfClans.Models.ClanWarAttack> allAttacks =
                cWar.GetAllAttacks();
            IReadOnlyCollection<ClashOfClans.Models.ClanWarAttack> allOpens =
                cWar.GetAllOpeningAttacks();

            cWar.Clan.Members?.AsParallel()
                .ForAll(m =>
                {
                    Player? dbPlayer = players.FirstOrDefault(
                        p =>
                            (p.Guild == guild || p.Guild.Id == Configurations.DEV_GUILD_ID)
                            && p.Tag == m.Tag
                    );
                        ...
                        .Select(
                            a =>
                                new PlayerStatistic(
                                    ...
                                    a.Order,
                                    warType,
                                    PlayerStatisticType.ATTACK,
                                    allOpens.Contains(a)
                                        ? PlayerStatisticAction.OPENING
                                        : PlayerStatisticAction.RUNNING_OVER,
                                    a.Stars,
        ...
        private static async Task ScanCurrentWarAsync(IGuild guild, ClashOfClans.Models.ClanWar war)
        {
            ...
            Competition? competition = dbCompetitions.FirstOrDefault(
                c => c.Name == match?.CompetitionName
            );
            ...
            WarStatistic currentWar =
                new(
                    ...
                    war.GetAverageAttackTime().TotalSeconds,
                    war.Opponent.Stars,
                    war.Opponent.DestructionPercentage,
                    war.GetAverageDefenseTime().TotalSeconds
                );

            // If the last war added is the current, do nothing
            if (lastWar == currentWar)
                return;
            ...
            if (isEsport)
                await DisplayResultAsync(guild, competition, war);

            if (dbGuild.PremiumLevel >= PremiumLevel.MEDIUM && !isEsport)
                InsertPlayerStatistics(dbGuild, war, warType);
            if (dbGuild.PremiumLevel >= PremiumLevel.LOW && isEsport)
                InsertPlayerStatistics(dbGuild, war, warType);
        }
    }
}
